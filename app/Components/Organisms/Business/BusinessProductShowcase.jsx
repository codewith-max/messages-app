...
function CardIcon({ type }) {
  if (type === 'briefcase') {
    return (
      <span className="inline-flex h-12 w-12 items-center justify-center text-[#25D366]" aria-hidden>
    ...
    <>
      <section className="bg-white px-4 py-16 sm:px-6 md:py-24 lg:px-8" aria-labelledby="business-products-heading">
        <div className="mx-auto max-w-[900px] text-center">
          <p className="text-[15px] font-medium text-[#111b21]/70 md:text-[16px]">{productsIntro.eyebrow}</p>
          <h2 id="business-products-heading" className="mt-4 text-[24px] font-semibold leading-snug text-[#1c1e21] sm:text-[28px] md:text-[32px] md:leading-[1.25]">
            ...
            <span className="border-b-[3px] border-[#25D366] pb-0.5 decoration-[#25D366]">{productsIntro.titleUnderline}</span>
        ...
        <div className="mx-auto grid max-w-[1100px] gap-6 md:grid-cols-2 md:gap-8">
          {productCards.map((card) => (
            ...
            >
              ...
              <h3 className="mt-6 text-[22px] font-bold text-[#111b21] md:text-[24px]">{card.title}</h3>
              <p className="mt-3 flex-1 text-[16px] leading-relaxed text-[#111b21]/80">{card.description}</p>
              <Link
                href={card.href}
                className="mt-8 inline-flex items-center gap-2 text-[15px] font-semibold text-[#25D366] transition hover:text-[#20bd5c]"
              >
                {card.linkText}
