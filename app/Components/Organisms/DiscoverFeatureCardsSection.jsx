        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-stretch lg:gap-14 xl:gap-16">
          <div className="flex shrink-0 flex-col justify-between lg:w-[34%] xl:w-[32%]">
            <div>
              ...
              <p className="mt-5 max-w-lg text-[17px] leading-relaxed text-[#111b21]/88 md:text-[18px]">{subtitle}</p>
            </div>
            <div className="mt-8 flex gap-3 lg:mt-10">
              <button
                type="button"
                onClick={() => scrollByDir(-1)}
                className={`inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-[#111b21] text-[#111b21] shadow-sm transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] focus-visible:ring-offset-2 ${navSurfaceClass} ${navHoverClass} ${navRingOffsetClass}`}
          ...
          <div className="min-w-0 flex-1 lg:pt-1">
            ...
            >
              {cards.map((card) => (
                <article
                  key={card.id}
                  className="flex w-[min(88vw,22rem)] shrink-0 snap-start flex-col overflow-hidden rounded-[26px] bg-white shadow-[0_4px_24px_rgba(0,0,0,0.06)] ring-1 ring-black/[0.06] sm:w-[min(85vw,24rem)]"
                >
