      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-12">
          <Link href="/" className="flex-shrink-0 flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-xl font-bold tracking-tight text-[#25D366]">WhatsApp</span>
          </Link>

          <nav className="hidden lg:flex items-center gap-8">
            <Link href="/features" className="flex items-center gap-1 cursor-pointer group">
              <span
                className={`text-[15px] transition-colors ${isDarkNav ? 'text-white hover:text-[#25D366]' : 'text-[#111b21] hover:text-[#25D366]'}`}
              >
                {h.features}
              </span>
              <svg
                className={`w-4 h-4 transition-transform ${isDarkNav ? 'text-white group-hover:text-[#25D366]' : 'text-[#111b21] group-hover:text-[#25D366]'} ${isFeaturesPage ? 'rotate-180' : 'group-hover:rotate-180'}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="#download"
              className={`flex items-center gap-2 text-[15px] font-medium text-[#111b21] rounded-full bg-[#25D366] px-6 py-2.5 hover:bg-[#20bd5a] transition-all hover:shadow-[0_4px_14px_0_rgba(37,211,102,0.39)] ${
                isDarkNav ? 'border-0' : 'border border-[#111b21]'
              }`}
            >
